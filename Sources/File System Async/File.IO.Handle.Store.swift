...
extension File.IO.Handle {
    ...
    ///
    /// ## Design
    /// - Dictionary maps Handle.ID → File.IO.Handle.Box
    /// - Store mutex guards map mutations and shutdown state
    /// - Per-handle locks (in File.IO.Handle.Box) guard handle operations
    ...
    final class Store: @unchecked Sendable {
        ...
        private static let scopeCounter = File.IO.Atomic.Counter()
...
extension File.IO.Handle.Store {
    ...
    /// - Throws: `Executor.Error.shutdownInProgress` if store is shut down.
    ///
    /// ## Implementation Note
    ...
    /// 2. Create File.IO.Handle.Box (consumes handle)
    /// 3. Atomically register under lock (with double-check for shutdown race)
    ///
    /// If shutdown races between phase 1 and 3, the File.IO.Handle.Box's deinit will
    /// close the handle as a safety net.
    func register(_ handle: consuming File.Handle) throws -> File.IO.Handle.ID {
        ...
        if alreadyShutdown {
            _ = try? handle.close()
            throw File.IO.Executor.Error.shutdownInProgress
        }

        // Phase 2: Create box (consumes handle) - outside lock
        let box = File.IO.Handle.Box(handle)
        ...
        return try state.withLock { state in
            ...
            let id = File.IO.Handle.ID(raw: state.nextID, scope: scope)
            state.nextID += 1
    ...
    func withHandle<T>(_ id: File.IO.Handle.ID, _ body: (inout File.Handle) throws -> T) throws -> T {
        ...
        let box: File.IO.Handle.Box? = state.withLock { state in
            state.handles[id]
        }
    ...
    func destroy(_ id: File.IO.Handle.ID) throws {
    ...
    func shutdown() {
        ...
        let remainingHandles: [File.IO.Handle.ID: File.IO.Handle.Box] = state.withLock { state in
    ...
    func isValid(_ id: File.IO.Handle.ID) -> Bool {
