...
extension File.System.Stat.Test.Unit {
    ...
    func existsReturnsFalseForNonExisting() throws {
        try File.Directory.temporary { dir in
            let filePath = File.Path(dir.path, appending: "non-existing")
    ...
    func lstatInfoReturnsDirectoryForDirectory() throws {
        try File.Directory.temporary { dir in
            ...
            let info = try File.System.Stat.lstatInfo(at: subDir)
            #expect(info.type == .directory)
        }
    }
    ...
    func infoReturnsCorrectTypeForFile() throws {
        try File.Directory.temporary { dir in
            let filePath = File.Path(dir.path, appending: "test.txt")
            try File.System.Write.Atomic.write(Array("Hello, World!".utf8).span, to: filePath)
...
extension File.System.Stat.Test.Performance {
    ...
    func statInfo() throws {
        ...
        let filePath = File.Path(td.path, appending: "perf_stat_\(Int.random(in: 0..<Int.max)).txt")
